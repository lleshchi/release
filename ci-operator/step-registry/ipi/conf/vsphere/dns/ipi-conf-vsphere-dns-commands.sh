...
if "$VSPHERE_ADDITIONAL_CLUSTER"; then
  # Spoke
  ...
  echo "export NETWORK_NAME=$network_name" > "$env_file" #FIXME
  echo "export VSPHERE_CLUSTER=DEVQEcluster" > "$env_file" #FIXME
  echo "export VCENTER=$VCENTER" > "$env_file" #FIXME
